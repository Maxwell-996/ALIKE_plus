...
def draw_lines(img, nb_lines=10):
    ...
    segments = np.empty((0, 4), dtype=np.int64)
...
def draw_ellipses(img, nb_ellipses=20):
    ...
    rads = np.empty((0, 1), dtype=np.int64)
...
def draw_checkerboard(img, max_rows=7, max_cols=7, transform_params=(0.05, 0.15)):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            colors[i * cols + j] = col
            cv2.fillConvexPoly(img, np.array([(warped_points[i * (cols + 1) + j, 0],
...
def draw_stripes(img, max_nb_cols=13, min_width_ratio=0.04,
    ...
    for i in range(col):
        ...
        cv2.fillConvexPoly(img, np.array([(warped_points[i, 0],
                                          warped_points[i, 1]),
                                         (warped_points[i+1, 0],
                                          warped_points[i+1, 1]),
    ...
    for _ in range(nb_rows):
        ...
        color = get_random_color(background_color)
        cv2.line(img, (warped_points[row_idx + col_idx1, 0],
    ...
    for _ in range(nb_cols):
        ...
        color = get_random_color(background_color)
        cv2.line(img, (warped_points[col_idx, 0],
...
def draw_cube(img, min_size_ratio=0.2, min_angle_rot=math.pi / 10,
    ...
    for i in [0, 1, 2]:
        cv2.fillPoly(img, [cube[faces[i]].reshape((-1, 1, 2))],
                    col_face)
    ...
    for i in [0, 1, 2]:
        for j in [0, 1, 2, 3]:
            col_edge = (col_face + 128
                        ...
                        % 256 
            cv2.line(img, (cube[faces[i][j], 0], cube[faces[i][j], 1]),
