...
class TrainWrapper(ALIKE):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 参照superpoint模型设计出的误差
        if self.mgpt_dector_loss >0:
            loss += self.mgpt_loss(pred0['scores_map'].squeeze(1),batchpt['keypoints0_map'].squeeze(1),\
                                    dustbin = pred0['dustbin'].squeeze(1))
            loss += self.mgpt_loss(pred1['scores_map'].squeeze(1),batchpt['keypoints1_map'].squeeze(1),\
                               dustbin = pred1['dustbin'].squeeze(1))
        # scoremap的MSE误差
        if self.mgpt_MSE_loss >0:
            loss += self.MSE_loss(pred0['scores_map'].squeeze(1),batchpt['keypoints0_map'].squeeze(1))
            loss += self.MSE_loss(pred1['scores_map'].squeeze(1),batchpt['keypoints1_map'].squeeze(1))
