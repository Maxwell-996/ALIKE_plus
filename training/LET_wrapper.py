...
class LET_Wrapper(LET):
    def __init__(self,
                 ...
                 # ================================== detect parameters
                 radius: int = 2,
                 top_k: int = 50, scores_th: float = 0, n_limit: int = 0,  # in training stage
                 scores_th_eval: float = 0.2, n_limit_eval: int = 5000,  # in evaluation stage
                 # ================================== gt reprojection th
                 train_gt_th: int = 5, eval_gt_th: int = 3,
                 # ================================== loss weight
                 w_pk: float = 0,  # weight of peaky loss
                 w_rp: float = 0,  # weight of reprojection loss
                 w_sp: float = 0,  # weight of score map repetability loss
                 w_ds: float = 0,  # weight of descritptor loss
                 w_triplet: float = 0.,
                 sc_th: float = 0,  # score threshold in peaky and  reprojection loss
                 norm: int = 0,  # distance norm
                 temp_sp: float = 0.,  # temperature in ScoreMapRepLoss
                 temp_ds: float = 0.,  # temperature in DescReprojectionLoss
                 # ================================== learning rate
                 lr: float = 3e-3,
        ...
        self.mgpt_dector_loss = 1.
        self.mgpt_MSE_loss = 0
    ...
    def compute_correspondence(self, pred0, pred1, batch, rand=True):
        ...
        for idx in range(b):
            ...
            similarity_map_01  = None
            similarity_map_10 = None
            # similarity_map_01_valid = similarity_map_01[ids0]  # valid descriptors
            # similarity_map_10_valid = similarity_map_10[ids1]
            similarity_map_01_valid = None  # valid descriptors
            similarity_map_10_valid = None
    ...
    def val_match(self, batch,batch_idx):
        ...
        self.top_k, self.scores_th, self.n_limit = 0, 0.015, 200
        ...
        try:
            ...
            # loss = 0
            # if self.mgpt_dector_loss > 0:
            #     loss += self.mgpt_loss(pred0['ini_smp'].squeeze(1),batch['keypoints0_map'].squeeze(1),\
            #                         dustbin = pred0['dustbin'].squeeze(1))
            #     loss += self.mgpt_loss(pred1['ini_smp'].squeeze(1),batch['keypoints1_map'].squeeze(1),\
            #                     dustbin = pred1['dustbin'].squeeze(1))
            # if self.mgpt_MSE_loss > 0:
            #     loss += self.MSE_loss(pred0['ini_smp'].squeeze(1),batch['keypoints0_map'].squeeze(1))
            #     loss += self.MSE_loss(pred1['ini_smp'].squeeze(1),batch['keypoints1_map'].squeeze(1))
            ...
            if np.max(ori) <= 1:
                ori = ori*255
            smp = smp/np.max(smp)*255
            if "keypoints0_map" in batch.keys():
                kmap = batch["keypoints0_map"][0].unsqueeze(-1).repeat(1,1,3).cpu().numpy()*255
                target = np.concatenate([ori,kmap],axis = 1)
                cv2.imwrite(save_path+"{}_ori.png".format(self.counts),target)
            else:
            ...
            return torch.tensor([[0]]), num_feat, 0, self.tempcounter, 0, 0
            # return (dist,
